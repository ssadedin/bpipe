...
- [ ] Add `stub` field to `OutputMetaData` (persisted as a property in the metadata file)
- [ ] Add stub detection utility method (`hasStubInputs`) that checks if any resolved inputs are stubs
- [ ] Implement stub file creation logic (touch files + save metadata with `stub=true`)
- [ ] Wire stub option into `PipelineStage.waitForDevInteraction()` (detect "stub" in response file)
- [ ] Add automatic stub propagation check in `PipelineContext.execImpl()` before command execution
