...
touch test.csv
run test.txt
[ `grep -c "Skipping steps" test.out ` == 3 ] || err "Failed to find 3 counts of phrase Skipping steps after touching test.csv"
...
grep -q 'Skipping steps to create test.xml'  test.out && err 'Incorrectly skipped creating file test.xml, even though an input was updated'
grep -q 'Skipping steps to create test.xls' test.out || err 'Failed to skip creating file test.xls even though the input that was updated is not one it depends on'
