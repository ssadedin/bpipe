...
grep -q 'Skipping' test.out && err "Incorrectly skipped some commands on first (clean) run"

run test1.gz test2.gz

grep -q 'Skipping steps to create' test.out || err "Failed to skip all the steps"

rm *.bam

run test1.gz test2.gz

grep -q 'Skipping steps to create' test.out && err "Incorrectly skipped whole transform when some outputs missing"
grep -q 'Skipping command gunzip' test.out || err "Failed to skip first commands in transform even thought outputs up to date"
